...
// GetUnfallCount godoc
//
// @Summary      Anzahl der Verkehrsunfälle	mit Personenschaden im Unfallatlas abrufen
// @Description  Gibt die Anzahl aller Unfälle zurück, die den angegebenen Filterkriterien entsprechen.
// @Tags         Unfallatlas
// @Accept       json
// @Produce      json
// @Param        bundesland       				query    string  false  "Filter nach Bundesland" enums(Baden-Württemberg, Bayern, Berlin, Brandenburg, Bremen, Hamburg, Hessen, Mecklenburg-Vorpommern, Niedersachsen, Nordrhein-Westfalen, Rheinland-Pfalz, Saarland, Sachsen, Sachsen-Anhalt, Schleswig-Holstein, Thüringen)
...
// @Param        min_lon          				query    float64 false  "Minimum Longitude"
// @Param        max_lon          				query    float64 false  "Maximum Longitude"
// @Success      200              				{object} map[string]int "e.g. {\"count\": 42}"
// @Failure      400         					{object} HTTPError "Bad Request - Invalid parameter type or range"
// @Failure      500         					{object} HTTPError "Internal Server Error - Database execution or scanning failure"
// @Router       /unfall/count [get]
func (h *AccidentHandler) GetUnfallCount(c *gin.Context) {
	baseQuery := `SELECT COUNT(*) FROM unfall`
	...
	// execute query
	var count int
	err := h.DB.QueryRow(finalQuery, queryArgs...).Scan(&count)
	if err != nil {
		c.JSON(http.StatusInternalServerError, HTTPError{Error: err.Error()})
		return
	}
...
// @Failure      500         {object}  HTTPError "Internal Server Error - Database execution or scanning failure"
// @Router       /unfall/jahre [get]
func (h *AccidentHandler) GetUnfallJahre(c *gin.Context) {
	query := `SELECT DISTINCT jahr FROM unfall ORDER BY jahr`
