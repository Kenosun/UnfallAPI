...
// @Param        beteiligungsart  query    string  false  "Filter nach Beteiligungsart" enums(Unfallbeteiligte, Hauptverursacher des Unfalls)
...
// @Success      200              {array}  data.UnfallBeteiligung
...
// @Router       /unfallBeteiligung [get]
func (h *AccidentHandler) GetUnfallBeteiligung(c *gin.Context) {
	baseQuery := `SELECT verkehrsart, unfallkategorie, ortslage, geschlecht, altersgruppe, beteiligungsart, jahr, monat, anzahl FROM unfall_beteiligung`
	...
	stringParams := map[string]string{
		...
		"beteiligungsart": c.Query("beteiligungsart"),
	}
	...
	// JSON response
	var results []data.UnfallBeteiligung
	for rows.Next() {
		var s data.UnfallBeteiligung
		err := rows.Scan(&s.Verkehrsart, &s.Unfallkategorie, &s.Ortslage, &s.Geschlecht, &s.Altersgruppe, &s.Beteiligungsart, &s.Jahr, &s.Monat, &s.Anzahl)
		if err != nil {
			c.JSON(http.StatusInternalServerError, HTTPError{Error: err.Error()})
			return
		}
...
// GetUnfallBeteiligungJahre godoc
//
...
// @Failure      500         {object}  HTTPError "Internal Server Error - Database execution or scanning failure"
// @Router       /unfallBeteiligung/jahre [get]
func (h *AccidentHandler) GetUnfallBeteiligungJahre(c *gin.Context) {
	query := `SELECT DISTINCT jahr FROM unfall_beteiligung ORDER BY jahr`
