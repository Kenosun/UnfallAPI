...
// @Param        fehlverhalten   	query    string  false  "Filter nach Fehlverhalten"
...
// @Success      200         		{array}  data.UnfallFehlverhalten
...
// @Router       /unfallFehlverhalten [get]
func (h *AccidentHandler) GetUnfallFehlverhalten(c *gin.Context) {
	baseQuery := `SELECT verkehrsart, fehlverhalten, jahr, monat, anzahl FROM unfall_fehlverhalten`
	...
	stringParams := map[string]string{
		...
		"fehlverhalten": c.Query("fehlverhalten"),
	}
	...
	// JSON response
	var results []data.UnfallFehlverhalten
	for rows.Next() {
		var s data.UnfallFehlverhalten
		err := rows.Scan(&s.Verkehrsart, &s.Fehlverhalten, &s.Jahr, &s.Monat, &s.Anzahl)
		if err != nil {
			c.JSON(http.StatusInternalServerError, HTTPError{Error: err.Error()})
			return
		}
...
// GetUnfallFehlverhaltenJahre godoc
//
...
// @Failure      500         {object}  HTTPError "Internal Server Error - Database execution or scanning failure"
// @Router       /unfallFehlverhalten/jahre [get]
func (h *AccidentHandler) GetUnfallFehlverhaltenJahre(c *gin.Context) {
	query := `SELECT DISTINCT jahr FROM unfall_fehlverhalten ORDER BY jahr`
